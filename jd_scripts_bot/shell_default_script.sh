...
if [ "$RANDOM_DELAY_MAX" ]; then
  if [ "$RANDOM_DELAY_MAX" -ge 1 ]; then
    ...
    sed -i "/\(jd_xtg.js\|jd_cash_exchangecdle.js\|jd_cfd_loop.js\|jd_blueCoin.js\|jd_joy_reward.js\|jd_car_exchange.js\|docker_entrypoint.sh\|monkcoder_adolf_star.js\)/!s/node/sleep \$((RANDOM % \$RANDOM_DELAY_MAX)); node/g" $mergedListFile
  fi
else
...
echo "添加一些可以并发启动的脚本"
sed -i "/\(jd_joy_reward.js\|jd_blueCoin.js\|jd_jxcfdtx.js\|jd_car_exchange.js\|lzz_super_redrain.js\|lzz_half_redrain.js\|jd_cash_exchangecdle.js\)/s/node/spnode conc/g" $mergedListFile
crontab $mergedListFile
