...
function main(){
    ...
    d_jsnum=$(ls -l /scripts | grep -oE "^-.*js$" | wc -l)
    d_jsname=$(ls -l /scripts | grep -oE "^-.*js$" | grep -oE "[^ ]*js$")
    # DIY任务
    diycron
    # DIY脚本更新TG通知
    info_more=$(echo $a_jsname  $b_jsname | tr " " "\n" | sort | uniq -c | grep -oE "1 .*$" | grep -oE "[^ ]*js$" | tr "\n" " ")
    info_more1=$(echo $c_jsname  $d_jsname | tr " " "\n" | sort | uniq -c | grep -oE "1 .*$" | grep -oE "[^ ]*js$" | tr "\n" " ")
    [[ "$a_jsnum" == "0" || "$a_jsnum" == "$b_jsnum" ]] || curl -sX POST "https://api.telegram.org/bot$TG_BOT_TOKEN/sendMessage" -d "chat_id=$TG_USER_ID&text=和尚脚本更新完成：$a_jsnum $b_jsnum $info_more" >/dev/null
    [[ "$c_jsnum" == "0" || "$c_jsnum" == "$d_jsnum" ]] || curl -sX POST "https://api.telegram.org/bot$TG_BOT_TOKEN/sendMessage" -d "chat_id=$TG_USER_ID&text=京东到家脚本更新完成：$c_jsnum $d_jsnum $info_more1" >/dev/null
    # LXK脚本更新TG通知
